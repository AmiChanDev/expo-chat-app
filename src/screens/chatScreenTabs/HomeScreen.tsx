...
export default function HomeScreen() {
        ...
        navigation.setOptions({
            ...
            headerRight: () => (
                <View className="flex-row gap-4">
                    <TouchableOpacity
                        className={`p-2 rounded-lg ${isDark ? "bg-gray-700" : "bg-white/80"}`}
                        activeOpacity={0.7}
                    >
                        <Ionicons name="camera-outline" size={22} color={isDark ? "#d1d5db" : "#374151"} />
                    </TouchableOpacity>
                    <TouchableOpacity
                        className={`p-2 rounded-lg ${isDark ? "bg-gray-700" : "bg-white/80"}`}
                        activeOpacity={0.7}
                    ...
                    >
                        <Ionicons name="ellipsis-vertical" size={22} color={isDark ? "#d1d5db" : "#374151"} />
                    </TouchableOpacity>
                </View>
            ),
        });
    ...
    const renderItem = ({ item }: { item: Chat }) => (
        <TouchableOpacity
            className={`${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} rounded-2xl my-1 mx-1 shadow-sm border`}
            activeOpacity={0.7}
            ...
            <View className="flex-row items-center p-4">
                ...
                <View className="flex-1">
                    <View className="flex-row justify-between items-center mb-1">
                        <Text className={`text-base font-semibold ${isDark ? "text-white" : "text-gray-800"} flex-1 mr-2`} numberOfLines={1}>
                            {item.friendName}
                        </Text>
                        <Text className={`text-xs ${isDark ? "text-gray-400" : "text-gray-400"} font-medium`}>
                    ...
                    <View className="flex-row justify-between items-center">
                        <Text
                            className={`text-sm ${isDark ? "text-gray-300" : "text-gray-600"} flex-1 mr-2`}
                            numberOfLines={1}
                            ellipsizeMode="tail"
                        >
                            {item.lastMessage}
    ...
    return (
        <SafeAreaView className={`flex-1 ${isDark ? "bg-gray-900" : "bg-gray-50"}`}>
            {/* Search Bar */}
            <View className={`px-4 ${isDark ? "bg-gray-900" : "bg-slate-50/80"}`}>
                <View className={`px-2 flex-row items-center ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} rounded-2xl py-2 shadow-sm border`}>
                    <Ionicons name="search" size={20} color={isDark ? "#9ca3af" : "#9ca3af"} />
                    <TextInput
                        className={`px-1 flex-1 text-base ${isDark ? "text-gray-200" : "text-gray-700"}`}
                    ...
                    {search.length > 0 && (
                        <TouchableOpacity
                            onPress={() => setSearch("")}
                            className={`p-1 rounded-full ${isDark ? "active:bg-gray-700" : "active:bg-gray-100"}`}
                            activeOpacity={0.7}
                        >
            ...
            <FlatList
                ...
                ListEmptyComponent={() => (
                    <View className="flex-1 justify-center items-center py-20 px-8">
                        ...
                        <Text className={`text-sm ${isDark ? "text-gray-400" : "text-gray-500"} text-center leading-6 mb-6`}>
                            {chatList.length === 0
                        ...
                        <View className={`${isDark ? "bg-gray-800 border-gray-700" : "bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-100"} p-4 rounded-xl border`}>
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        >
                            <Text style={{ fontSize: 16, color: isDark ? "#f9fafb" : "#000" }}>Settings</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={{ padding: 12 }}
                            onPress={() => {
                        ...
                        >
                            <Text style={{ fontSize: 16, color: isDark ? "#f9fafb" : "#000" }}>My Profile</Text>
                        </TouchableOpacity>
                    </View>
                </Pressable>
            </Modal>
